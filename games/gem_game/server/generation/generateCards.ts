...
import { GemGameConfig, Card, Cost, Color } from '../../shared/types';

const COLORS: Color[] = ['diamond', 'ruby', 'emerald', 'sapphire', 'onyx'];
...
function generateCost(colorsCount: number, totalLimit: number): Cost {
...
function generateImagePath(color: string): string {
  ...
  return `/games/gem_game/img/${color}/${color}_${num}.jpg`;
}
...
function generateBalancedDeck(size: number, generator: () => Card): Card[] {
...
export function generateDecks(config: GemGameConfig) {
