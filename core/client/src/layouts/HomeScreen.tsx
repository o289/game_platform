...
export default function InitScreen({
  ...
  return (
    <>
      ...
      {/* ルーム作成 */}
      <button
        className={`w-full max-w-xs px-6 py-3 text-base rounded-lg font-semibold transition-all ${
          isNameValid
            ? 'bg-gradient-to-r from-green-400 to-emerald-500 hover:scale-105 shadow-lg'
      ...
      <div className="flex flex-col gap-3 items-center w-full">
        ...
        <button
          className={`w-full max-w-xs px-6 py-3 text-base rounded-lg font-semibold transition-all ${
            isNameValid && joinId
              ? 'bg-gradient-to-r from-blue-400 to-cyan-500 hover:scale-105 shadow-lg'
              : 'bg-gray-500 cursor-not-allowed'
          }`}
